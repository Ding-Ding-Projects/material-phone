...
- Historical build-only evidence exercised the former `tar.xz` fallback through download, SHA verification, local extraction, staging, bundled `pacman.exe`, and signed keyring/database work. Hosted runs later disproved that route: absolute Windows `tar.exe` remained active until both its 300-second and 1,200-second deadlines, so the former archive route is no longer active.
...
- The active MSYS2 fallback is the official immutable `msys2-base-x86_64-20260611.sfx.exe` release asset at `https://github.com/msys2/msys2-installer/releases/download/2026-06-11/msys2-base-x86_64-20260611.sfx.exe`. Its exact size is 52,898,952 bytes, its manifest ceiling is 56,000,000 bytes, and its SHA-256 is `c105946e64e08f099ac0e4647461ce762b95333ad211777666476a9a41451d65`; the digest was independently confirmed from the release's official `.sha256` asset and matches GitHub release metadata. Its finite 1,200-second download deadline covers the exact artifact at the established conservative 48 KiB/s floor plus setup margin, while the existing HTTPS and redirect-host allowlists remain enforced.
- The verified official invocation contract is `msys2-base-x86_64-20260611.sfx.exe -y -o<directory>`, as documented by the upstream `msys2-installer` README. Bootstrap runs that exact downloaded executable through an absolute path, reports 15-second heartbeats, terminates it after a finite 600 seconds, removes partial output, and accepts only one case-exact `msys64` root containing `usr/bin/pacman.exe`. It then retains the existing same-volume, 60-second bounded staging move. The corrected Graphviz package, bundled keyring initialization, two-phase runtime upgrade, exact package set, and no-signing packaging behavior are unchanged.
